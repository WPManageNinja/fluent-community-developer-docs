---
title: Course Model
description: ORM reference for FluentCommunity\App\Models\Course.
---
...
| `creator()` | `hasOneThrough` | `XProfile` | Signature: `—` |
| `students()` | `belongsToMany` | `User` | Signature: `—` |
| `course_topics()` | `hasMany` | `CourseTopic` | Signature: `—` |
| `enrollment()` | `belongsTo` | `SpaceUserPivot` | Signature: `—` |
| `student_enrollments()` | `hasMany` | `SpaceUserPivot` | Signature: `—` |
| `group()` | `belongsTo` | `SpaceGroup` | Signature: `—` |
| `categories()` | `belongsToMany` | `Term` | Signature: `—` |
...
| `ByAdminAccess` | `$query, $userId = null` | By Admin Access scope declared on the model. |
| `SearchBy` | `$query, $search` | Search By scope declared on the model. |
| `ByPostTopic` | `$query, $topicSlug = null` | By Post Topic scope declared on the model. |
...
| `syncCategories()` | sync Categories implemented on the model. |
| `isCourseAdmin()` | is Course Admin implemented on the model. |
